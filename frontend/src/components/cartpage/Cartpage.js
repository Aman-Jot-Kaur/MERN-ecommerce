...
const CartPage = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`http://localhost:3001/getcart?q=${email}`).then((res) => {
                ...
                const updatedProducts = res.data.cart.map((product) => {
                    return {
                        ...product,
                        quantity: isNaN(product.quantity) || product.quantity < 0 ? 0 : Math.floor(product.quantity)
                    };
                });
    ...
    const handleQuantityChange = (_id, quantity) => {
        ...
        const updatedProducts = products.map((product) => {
            if (product._id === _id) {
                const updatedQuantity = isNaN(quantity) || quantity < 0 ? 0 : Math.floor(quantity);
